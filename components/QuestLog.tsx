...
export default function QuestLog() {
  ...
  useEffect(() => {
    const fetchQuests = async () => {
      const { data } = await supabase.from('quests').select('*').eq('status', 'active').order('created_at', { ascending: false });
  ...
  return (
    <div className="p-6 rounded-2xl bg-black/40 border border-white/10 backdrop-blur-xl h-full min-h-[300px]">
      ...
      <div className="space-y-3">
        <AnimatePresence>
            {quests.length === 0 ? <p className="text-zinc-600 text-xs font-mono text-center py-4">NO ACTIVE DIRECTIVES</p> : 
            quests.map((quest) => (
            <motion.div
                key={quest.id}
