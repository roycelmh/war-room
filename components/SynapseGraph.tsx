...
export default function SynapseGraph() {
  ...
  useEffect(() => {
    const fetchGraph = async () => {
      const { data: nodes } = await supabase.from('memory_nodes').select('id, name, type').limit(50); 
      const { data: edges } = await supabase.from('memory_edges').select('source_id, target_id, relation').limit(100);

      if (nodes && edges) {
        const formattedNodes: GraphNode[] = nodes.map((n: any) => ({ id: n.id, name: n.name, group: n.type }));
        const formattedLinks: GraphLink[] = edges.map((e: any) => ({ source: e.source_id, target: e.target_id, name: e.relation }));
